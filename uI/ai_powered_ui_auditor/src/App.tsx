...
export default function App() {
    ...
    // ── ADDED: raw File object so FormData sends real file bytes ──────────
    ...
    // ── FIXED: accepts rawFile + profileOverride as direct args ───────────
    // React setState is async — reading `category` from state in the same
    // tick as setCategory gives the OLD value. Passing fresh values as
    // arguments bypasses that so the correct profile always reaches the API.
    const handleProcess = async (
        option: 'rules' | 'elements' | 'all',
        rawFile?: File,
        profileOverride?: string,
    ) => {
        switch (option) {
            case 'rules': {
                const activeFile    = rawFile    ?? uploadedRawFile
                const activeProfile = (profileOverride ?? category ?? 'universal').trim().toLowerCase()
                ...
                try {
                    ...
                    formData.append('profile_form', activeProfile)

                    console.log(`[App] Audit → file: ${activeFile.name}  profile: ${activeProfile}`)
                    ...
                    {step === 'upload' && (
                        <UploadPage
                            ...
                            onProcess={(data) => {
                                ...
                                setUploadedRawFile(data.file || null)
                                setCategory(data.category || 'universal')
                                // ── KEY FIX: pass file + category directly ────
                                // Do NOT read from state — setCategory above is
                                // async and category still holds the OLD value
                                // in this same tick.
                                handleProcess(
                                    'rules',
                                    data.file ?? undefined,
